...
describe('getCARoot', () => {
	it('calls mkcert-prebuilt with -CAROOT and encoding utf-8', () => {
		...
		expect(mockExecSync).toHaveBeenCalledWith('"/usr/local/bin/mkcert" -CAROOT', {
			encoding: 'utf-8',
		});
	});
...
describe('installCA', () => {
	...
	it('uses stdio ignore by default', () => {
		...
		expect(mockExecSync).toHaveBeenCalledWith(expect.any(String), { stdio: 'ignore' });
	});
...
describe('generate', () => {
	...
	it('reads rootCA-key.pem from the CAROOT directory', async () => {
		...
		expect(mockReadFile).toHaveBeenCalledWith(`${caRoot}/rootCA-key.pem`, {
			encoding: 'utf-8',
		});
	});
	...
	it('reads rootCA.pem from the CAROOT directory', async () => {
		...
		expect(mockReadFile).toHaveBeenCalledWith(`${caRoot}/rootCA.pem`, { encoding: 'utf-8' });
	});
	...
	it('writes the key file to sslDir/<slug>.key', async () => {
		...
		expect(mockWriteFile).toHaveBeenCalledWith('/etc/ssl/certs/wp/my-env.key', 'KEY_CONTENT');
	});
