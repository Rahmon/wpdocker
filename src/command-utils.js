...
exports.checkForUpdates = async function () {
	...
	try {
		...
		console.error(
			chalk.yellow(
				'Failed to automatically check for updates. Please ensure WP Docker is up to date.',
			),
		);
	}
	...
	if (update) {
		console.warn(
			chalk.yellow(
				`WP Docker version ${update.latest} is now available. Please run \`npm i -g @wpdocker/wpdocker\` to update!`,
			),
		);
	}
};
