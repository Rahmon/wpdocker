...
async function deleteEnv(env, spinner) {
	...
	if ((await config.get('manageHosts')) === true) {
		try {
			...
			const sudoOptions = { name: 'WP Docker' };
			...
			await new Promise((resolve) => {
				...
				sudo.exec(
					`${node} "${hostsScript}" remove ${hostsToDelete}`,
					sudoOptions,
					(error, stdout) => {
						if (error) {
							if (spinner) {
								...
								console.error(
									`${chalk.bold.yellow('Warning: ')}Something went wrong deleting host file entries. There may still be remnants in /etc/hosts`,
								);
							}
						} else {
							if (spinner) {
