...
exports.handler = makeCommand(async function ({ env, verbose }) {
	...
	if (envSlug === false) {
		throw new Error(
			"Error: Unable to determine which environment to use snapshots with. Please run this command from within your environment's directory.",
		);
	}
	...
	const command = ['wp', 'snapshots'];
	for (let i = 0; i < process.argv.length; i++) {
		if (
			process.argv[i].toLowerCase() === 'wpsnapshots' ||
			process.argv[i].toLowerCase() === 'snapshots'
		) {
