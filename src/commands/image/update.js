...
async function removeBuiltImages(docker, spinner) {
	...
	const images = await docker
		.listImages({ filters: '{"label": ["com.10up.wp-local-docker=user-image"]}' })
		.catch(() => false);
...
exports.handler = makeCommand(
	{},
	async function ({ yes, verbose, removeBuiltImages: forceRemoveBuiltImages }) {
		if (!yes) {
