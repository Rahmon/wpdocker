...
exports.desc =
	'Migrates a V1 WP Docker environment to a new V2 environment. Before running this command, create a new environment using the `wpdocker create` command.';
...
async function exportOldDatabase(oldEnv, exportDir) {
	...
	try {
		execSync(
			`docker run -d --rm --name ${base} -v ${dataDir}:/var/lib/mysql -v ${exportDir}:/tmp/export mysql:5`,
			{ stdio: 'inherit' },
		);
		await waitForDB(base);
		console.log('Exporting old database');
		execSync(
			`docker exec ${base} sh -c "/usr/bin/mysqldump -u root -ppassword wordpress > /tmp/export/database.sql"`,
			{ stdio: 'inherit' },
		);
...
async function importNewDatabase(env, spinner) {
	...
	try {
		...
		execSync(
			'docker-compose exec --user www-data phpfpm wp db import /var/www/html/import/database.sql',
...
exports.handler = makeCommand(async function ({ old, env, verbose }) {
	...
	if (!(await fs.pathExists(path.join(oldEnv, 'docker-compose.yml')))) {
		throw new Error(
			'Could not find a docker-compose.yml file in the path specified for the old environment!',
		);
	}
	...
	console.log(
		' - wp-config.php has not been changed. Any custom configuration needs to be manually copied',
	);
	console.log(
		' - If you need to run a search/replace, run `wpdocker wp search-replace <olddomain> <newdomain>`',
	);
});
