...
exports.desc =
	'Opens a shell for a specified container in your current environment (Defaults to the phpfpm container).';
...
exports.handler = makeCommand(async ({ container, cmd, env, verbose }) => {
	...
	if (envSlug === false) {
		throw new Error(
			'Error: Unable to determine which environment to open a shell for. Please run this command from within your environment.',
		);
	}
