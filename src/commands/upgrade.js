...
exports.handler = makeCommand( { checkDocker: false }, async ( { verbose, env } ) => {
	...
	if ( phpVersion ) {
		if ( phpVersion === '5.5' ) {
			if ( spinner ) {
				spinner.warn( 'Support for PHP v5.5 was removed in the latest version of WP Docker.' );
				throw new Error( 'This environment cannot be upgraded. No changes were made.' );
			} else {
				console.warn( 'Support for PHP v5.5 was removed in the latest version of WP Docker.' );
	...
	try {
		...
		if ( spinner ) {
			spinner.succeed( `${ chalk.cyan( envSlug ) } is upgraded to the latest version of WP Docker` );
		} else {
