...
exports.handler = makeCommand({ checkDocker: false }, async ({ verbose, env }) => {
	...
	if (phpVersion) {
		if (phpVersion === '5.5') {
			if (spinner) {
				spinner.warn(
					'Support for PHP v5.5 was removed in the latest version of WP Docker.',
				);
	...
	if (yaml.services.elasticsearch !== undefined) {
		...
		if (elasticVersion === '7.9.3') {
			...
			const { upgradeElastic } = await inquirer.prompt({
				...
				type: 'confirm',
				message:
					'Do you want to upgrade elasticsearch image? This will delete all data on that volume. You will need to reindex.',
				default: false,
			});
	...
	yaml.services.phpfpm.volumes = volumes.reduce((acc, curr) => {
		if (deprecatedVolumes.includes(curr)) {
			...
			if (
				...
				acc.push(
					`./config/php-fpm/docker-php-ext-xdebug.ini:/etc/php/${phpVersionNumber}/fpm/conf.d/docker-php-ext-xdebug.ini:cached`,
				);
			}
		} else {
			acc.push(curr);
	...
	yaml.services.phpfpm.environment.ENABLE_XDEBUG =
		yaml.services.phpfpm.environment.ENABLE_XDEBUG || 'true';
	yaml.services.phpfpm.environment.PHP_IDE_CONFIG =
		yaml.services.phpfpm.environment.PHP_IDE_CONFIG || `serverName=${envSlug}`;
	...
	if (yaml.services.nginx && Array.isArray(yaml.services.nginx['depends_on'])) {
		yaml.services.nginx['depends_on'] = yaml.services.nginx.depends_on.filter(
			(service) => service !== 'memcacheadmin',
		);
	}
	...
	await fsExtra.copy(
		path.join(envUtils.srcPath, 'containers'),
		path.join(envPath, '.containers'),
	);
	...
	try {
		...
		if (spinner) {
			spinner.succeed(
				`${chalk.cyan(envSlug)} is upgraded to the latest version of WP Docker`,
			);
		} else {
			console.log(`${envSlug} is upgraded to the latest version of WP Docker...`);
		}
	} catch (err) {
		console.error(err);
	}
