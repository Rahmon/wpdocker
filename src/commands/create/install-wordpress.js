...
async function downloadWordPress(wordpressType, cwd, spinner) {
	...
	await compose.exec('phpfpm', 'wp core download --version=latest --force', {
		cwd,
		log: !spinner,
	});
...
async function install(hostname, wordpress, certs, cwd, spinner) {
	const { title, username, password, email, type } = wordpress;
...
async function emptyContent(cwd, spinner) {
	...
	await compose
		.exec('phpfpm', 'wp plugin delete hello akismet', { cwd, log: !spinner })
		.catch(() => {});
	await compose
		.exec(
			'phpfpm',
