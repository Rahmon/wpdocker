...
async function checkDockerCompose(envPath, slug, spinner) {
	...
	if (!yaml || !yaml.services || !yaml.services.nginx) {
		if (spinner) {
			spinner.warn(
				"Environment's docker-compose.yml file hasn't been found or it doesn't contain nginx service...",
			);
		} else {
			console.warn(
				"Environment's docker-compose.yml file hasn't been found or it doesn't contain nginx service.",
	...
	const { confirm } = await inquirer.prompt({
		...
		type: 'confirm',
		message:
			"Do you want to update environment's docker-compose.yml file to use the new certificate?",
		default: true,
	});
