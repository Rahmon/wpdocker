...
class Renderer {
	...
	getSpacer(token) {
		...
		return blockTags.includes(this.prevTag) && blockTags.includes(token.tag) ? EOL : '';
	}
			...
			switch (token.type) {
				...
				case 'paragraph_close': {
					...
					switch (prevToken.tag) {
						...
						case 'h6':
							result.push(
								chalk.bold.cyanBright(`${token.markup} `),
								chalk.bold.cyanBright(result.pop()),
								EOL,
							);
							break;
						case 'a': {
							const href = (prevToken.attrs.find(([attr]) => attr === 'href') ||
								[])[1];
